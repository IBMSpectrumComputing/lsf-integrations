...
#MANDATORY  GEDIT_CMD(GEDIT Executable Path) GEDIT_CMD: full path to the application executable, including command options. For example: /usr/bin/gedit
#MANDATORY  LM_LICENSE_FILE(License File Path) LM_LICENSE_FILE: full path to the FLUENT license file. For example: /share/app/fluent/Fluent.Inc/license/license.dat

if [ -z "$GEDIT_CMD" ] || [ -z "$LM_LICENSE_FILE" ] ; then  
