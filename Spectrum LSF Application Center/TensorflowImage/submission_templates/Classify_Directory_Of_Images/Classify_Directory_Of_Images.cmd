...
# call wrapper script, which will call classify_image.py
COMMANDTORUN=classify_image_directory.sh
...
if [ "x$JOB_NAME" != "x" ]; then
   #LSF_OPT="-J \"$JOB_NAME\""
   if [ "x$CONCURRENT" != "x" -a "$CONCURRENT" == "yes" ]; then
      LSF_OPT="-J \"$JOB_NAME[1-$NUM_IMAGES]\""
   else   # sequential
      LSF_OPT="-J \"$JOB_NAME[1-$NUM_IMAGES]%1\""
   fi
fi
#------------------------data options-----------------------------
