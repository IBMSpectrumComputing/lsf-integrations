...
if [ "x$NUM_STEPS" != "x" ] ; then
	OTHER_OPT="--how_many_training_steps $NUM_STEPS"
fi
...
if [ $result -eq 0 -a "x$TENSORBOARD" != "x" -a "$TENSORBOARD" == "yes" ]; then
	...
	JOB_RESULT2=`/bin/sh -c "bsub ${CWD_OPT} ${LSF_OPT2} ${ADVANCED_OPT2} tensorboard.sh ${OTHER_OPT2} 2>&1"`
	export JOB_RESULT2
fi
