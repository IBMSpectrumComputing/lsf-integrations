...
download(){
    ...
    if [[ ! -f zookeeper_${ZOOKEEPER_VERSION}.tgz ]]; then
        docker pull wurstmeister/zookeeper:${ZOOKEEPER_VERSION}
        if docker inspect wurstmeister/zookeeper:${ZOOKEEPER_VERSION} > /dev/null 2>&1; then
            docker save wurstmeister/zookeeper:${ZOOKEEPER_VERSION} | gzip > zookeeper_${ZOOKEEPER_VERSION}.tgz
        fi
    fi
